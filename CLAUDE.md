...
## 프로젝트 개요
- 서비스명: 피즐리 (Pizzly)
- 설명: 틈새시간(엘리베이터, 버스 대기 등)에 AI 캐릭터 피즐리가 눈 건강/손목 관리/목·어깨 관리/호흡·휴식 4개 영역의 미니 웰니스 퀘스트를 추천해주는 서비스
- 참가 트랙: 중앙해커톤 AAC(Anti-Aging Club) — 웰니스
- 형태: PWA (홈 화면 설치, 오프라인 기본 화면 지원)
...
- 인증: `Authorization: Bearer {JWT}` 헤더. 회원가입 없이 사전 준비된 테스트 계정으로 로그인(심사 기준상 로그인은 필수 평가 항목 아님, 캐릭터 성장 스토리 저장을 위해 계정 구분만 유지)
