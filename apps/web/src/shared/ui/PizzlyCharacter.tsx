...
import { ACCESSORY_ITEMS } from './accessory-items';
 ...
 * 보유 + 착용한 액세서리(모자·반다나 등)는 기본 곰 이미지 위에 겹쳐서 여러 개 동시에 보여준다.
 *
...
export function PizzlyCharacter({ level, size = 160, className }: PizzlyCharacterProps) {
  ...
  const alt = stage.name;

  const equippedAccessoryIds = accessoryStorage.getEquipped();
  const accessories = ACCESSORY_ITEMS.filter(
    (item) => equippedAccessoryIds.includes(item.id) && inventoryStorage.isOwned(item.shopItemId),
  );
    ...
    >
      <div className="pizzly-float h-full w-full">
        ...
        {accessories.map((item) => {
          const rect = item.rects[stage.minLevel as keyof typeof item.rects];
          return (
            <img
              key={item.id}
              src={item.image}
